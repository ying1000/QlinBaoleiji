...
$_CONFIG['LOGIN_FROM_MC_URL'] = file_get_contents("/tmp/".session_id().'_LOGIN_FROM_MC_URL');
...
if($_GET['controller']=='admin_interface'&&$_CONFIG['interfacesn_on']&&($_POST['sn']==$_CONFIG['interfacesn'] or $_GET['action']=='getQrcode'&&$_GET['sn']==$_CONFIG['interfacesn'])){
	...
	if($_GET['action']=='devLogin'){
		$r = json_decode(urldecode($_POST['data']), true);
		$minfo = $member->select_all("uid='".$r['memberid']."'");
	}else{
	...
	if($_GET['action']=='dev_login'){
		echo "window.parent.parent.location='".$LOGIN_FROM_MC_URL."admin.php?controller=admin_index&action=login'";
		exit;
	}elseif($_GET['action']=='async_account'||$_GET['action']=='async_config'||$_GET['action']=='recopyhtml'){
		echo "window.parent.location='".$LOGIN_FROM_MC_URL."admin.php?controller=admin_index&action=login'";
		exit;
	}
	go_url($LOGIN_FROM_MC_URL."admin.php?controller=admin_index&action=login&ref=".urlencode($ref), 1);
	exit;
...
if($_SESSION['ADMIN_UID']){
	ob_start();
	$_SESSION['urlprotocol'] = $_CONFIG['URLPROTOCOL'];				
...
if(file_exists(ROOT ."./controller/$controller.class.php")) {
	...
	if((!isset($_SESSION["ADMIN_LOGINED"]) || $_SESSION["ADMIN_LOGINED"] == false) && ($action != 'login_user_field' && $action != 'login' && $action !='chklogin' && $action != 'getpwd'&& $action != 'docronreports'&& $action != 'synchronization_ad_users'&&$action!='get_user_login_fristauth'&& $action != 'get_sms'&& $action != 'get_email')) {
		//go_url("admin.php?controller=admin_index&action=login");
		if($_GET['action']=='dev_login'){
			echo "window.parent.parent.location='".$_CONFIG['LOGIN_FROM_MC_URL']."admin.php?controller=admin_index&action=login'";
			exit;
		}
		elseif($_GET['action']=='async_account'||$_GET['action']=='async_config'||$_GET['action']=='recopyhtml'){
			echo "window.parent.location='".$LOGIN_FROM_MC_URL."admin.php?controller=admin_index&action=login'";
		...
		go_url($_CONFIG['LOGIN_FROM_MC_URL']."admin.php?controller=admin_index&action=login", 1);
	}
	else {
