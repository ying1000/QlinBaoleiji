...
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_index&action=main">系统密码</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li>
...
{{/if}}&nbsp;<input type="submit" height="35" align="middle" value=" 确定 " border="0" class="bnnew2"/>{{if $smarty.session.ADMIN_LEVEL eq 0}}{{if  $logintype ne 'apppub' and $logintype ne '_apppub'}}&nbsp;磁盘映射:<input type="checkbox" name="rdpdiskauth_" id="rdpdiskauth_" {{if !$member.rdpdiskauth_up}}disabled{{/if}} {{if $rdpdiskauth_up}}checked{{/if}} value="1"  />&nbsp;剪切版:<input type="checkbox" name="rdpclipauth_" id="rdpclipauth_" {{if !$member.rdpclipauth_up }}disabled{{/if}}  onclick="checkieNT52(this)" {{if $member.rdpclipauth_up }}checked{{/if}} value="1"  />&nbsp;本地:<input type="checkbox" name="consoleauth_" id="consoleauth_" {{if !$member.rdplocal}}disabled{{else}}{{if $member.rdplocalcheck }}checked{{/if}}{{/if}}  />&nbsp;<select  class="wbk"  name='fenbianlv' id='fenbianlv' > 
					...
					{{/if}}{{/if}}
					域名连接:<input type="checkbox" name="bydomain" id="bydomain" {{if $bydomain}}checked{{/if}} value="1"  />&nbsp;外网NAT IP:<input type="checkbox" name="outsideip" id="outsideip" {{if $outsideip}}checked{{/if}} value="1"  />&nbsp;
					{{/if}}			
</form>
	  </td>
  </tr>
		...
		{{if  $logintype ne 'apppub' and $logintype ne '_apppub'}}		
			...
			<tr bgcolor='{{if !$alldev[t].enable }}#cccccc{{elseif $alldev[t].passwordtry eq 1 or $alldev[t].passwordtry eq 2}}red{{elseif $smarty.section.t.index % 2 == 0}}f7f7f7{{/if}}' onmouseover="changeStyle(this,'o');" onmouseout="changeStyle(this,'{{if !$alldev[t].enable }}#cccccc{{elseif $alldev[t].passwordtry eq 1 or $alldev[t].passwordtry eq 2}}red{{elseif $smarty.section.t.index % 2 == 0}}f7f7f7{{/if}}');">{{if $smarty.session.ADMIN_LEVEL eq 1}}
				...
				<td class="td_line" width="30%">
					...
					<a id="d{{$alldev[t].id}}" href='admin.php?controller=admin_pro&action=dev_login&logintool=filezilla&id={{$alldev[t].id}}'  onclick="return goto3(this.id)" target="hide"><img width="16" height="16" src="{{$template_root}}/images/filezilla.png" title="FileZilla" border=0></a>
					{{else}}
...
<tr>
			...
			<td>{{$alldev[t].id}}</td>
				...
				<td></td>
				<td></td>
				<td><a id="a{{$i}}2"  href="#" onclick="return rdpOne({{$alldev[t].id}}, 0, false);" target="_blank"><img  src="{{$template_root}}/images/windows.jpg"  width=16 height='16' ></a>&nbsp;&nbsp;&nbsp;&nbsp;<a id="a_{{$alldev[t].id}}_pubclddsk"  href="#" title="公有云盘" onclick="return rdpgofolder({{$alldev[t].id}}, 'pubclddsk');" target="_blank"><img  src="{{$template_root}}/images/1_1.jpg"  width=16 height='16' ></a>&nbsp;&nbsp;&nbsp;&nbsp;<a id="a_{{$alldev[t].id}}_priclddsk"  href="#" title="私有云盘" onclick="return rdpgofolder({{$alldev[t].id}}, 'priclddsk');" target="_blank"><img  src="{{$template_root}}/images/1-1.png"  width=16 height='16' ></a></td>
			</tr>
			{{/section}}
			{{/if}}
	...
	for(var i=0; i<elms.length; i++){
		if(elms[i].name.substring(0,7)=='session'&&elms[i].checked){
			...
			if(args[0]=='RDP'){
				...
				var bydomain = document.getElementById('bydomain').checked ? '1' : '0';
				var outsideip = document.getElementById('outsideip').checked ? '1' : '0';
				var url = "admin.php?controller=admin_pro&action=dev_login&id="+args[1]+"&logintool=securecrt&type={{$type}}"+'&selectedip='+lbip+'&app_act='+app_act+'&bydomain='+bydomain+'&outsideip='+outsideip+'&'+Math.round(new Date().getTime()/1000);
			}
...
function rdpgo(iid,appdeviceid,isrdp){	
	...
	var weburl='admin.php?controller=admin_pro&action=dev_login&id='+iid+'&selectedip='+lbip{{if  !($logintype eq 'apppub' or $logintype eq '_apppub')}}+'&screen='+fenbian+'&app_act='+app_act+'&rdpclipauth='+rdpclipauth+'&rdpdiskauth='+rdpdiskauth{{/if}}+'&rdparg=0&appdeviceid='+appdeviceid+'&bydomain='+bydomain+'&outsideip='+outsideip;
	if(isrdp){
...
function rdpgo2(iid,appdeviceid,isrdp){
	...
	document.getElementById(aid).href='admin.php?controller=admin_pro&rdptype=activex&action=dev_login&id='+iid+'&selectedip='+lbip+'&rdparg=0&appdeviceid='+appdeviceid{{if  !($logintype eq 'apppub' or $logintype eq '_apppub')}}+'&screen='+fenbian+'&app_act='+app_act+'&rdpclipauth='+rdpclipauth+'&rdpdiskauth='+rdpdiskauth{{/if}}+'&bydomain='+bydomain+'&outsideip='+outsideip;
	if(isrdp){
...
function rdpgofolder(iid,t){
	...
	var url='admin.php?controller=admin_pro&action=dev_login&folder='+t+'&id='+iid+'&selectedip='+lbip+'&rdparg=0&appdeviceid=0'{{if  !($logintype eq 'apppub' or $logintype eq '_apppub')}}+'&screen='+fenbian+'&app_act='+app_act+'&rdpclipauth='+rdpclipauth+'&rdpdiskauth='+rdpdiskauth{{/if}}+'&bydomain='+bydomain+'&outsideip='+outsideip;
