...
AllServers[i]['device_ip']='{{$servers[kk].device_ip}}';
AllServers[i]['hostname']='{{$servers[kk].hostname}}';
i++;
{{/section}}
...
<ul>
	{{if 1 or $smarty.get.device_ip eq ''}}
...
<input type="password" name="hiddenpassword" id="hiddenpassword" style="display:none"/>	<table border=0 width=100% cellpadding=5 cellspacing=0 bgcolor="#FFFFFF" valign=top class="BBtable">
                  ...
                  <select  class="wbk"  name="serverid" id="device_ip">
					  ...
					  {{/if}}
                     	{{section name=k loop=$servers}}
							<option value="{{$servers[k].id}}" {{if $servers[k].device_ip == $p.device_ip or $servers[k].device_ip == $device_ip or $servers[k].hostname == $p.device_ip}}selected{{/if}}>{{$servers[k].device_ip}}-{{$servers[k].hostname}}</option>
						{{/section}}
                  </SELECT>&nbsp;&nbsp;&nbsp;&nbsp;
                  	<select  class="wbk"  name="ishostname" id="ishostname">
						<option value="0" {{if $p.ishostname == 0}}selected{{/if}}>IP连接</option>
						<option value="1" {{if $p.ishostname == 1}}selected{{/if}}>域名连接</option>
		  			</select>
		</td>
	</tr>
	{{if $appserverip eq ''}}
	...
	{{assign var="trnumber" value=$trnumber+1}}
	  <tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
		...
		<td>
			...
			<option value="{{$appprogram[p].id}}" autologinflag="{{$appprogram[p].autologin}}" id="program_{{$appprogram[p].name}}" {{if $pp.appprogramname eq $appprogram[p].name or $pp.autologinflag eq $appprogram[p].autologin}}selected{{/if}} >{{$appprogram[p].name}} </option>
			{{/section}}
			</select>
		</td>
	  </tr>
	{{assign var="trnumber" value=$trnumber+1}}
	  ...
	  {{assign var="trnumber" value=$trnumber+1}}
					<tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}} id="sqlserverport" >
		<td width="10%" align=right>SQL SERVER端口</td>
		<td><input type="text" size="100" name="sqlserver_port" value="{{if $p.id}}{{$p.sqlserver_port}}{{else}}1433{{/if}}" /></td>
	  </tr>
	   {{assign var="trnumber" value=$trnumber+1}}
	   ...
	   {{assign var="trnumber" value=$trnumber+1}}
					<tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}} id="oracle_db_tr" style="display:none">
		<td width="10%" align=right>Oracle Service_name</td>
		<td><input type="text" size="30" name="oracle_db" value="{{$p.oracle_db}}" /></td>
	  </tr>
	   {{assign var="trnumber" value=$trnumber+1}}
					<tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}} id="oracle_sqlserver_port_tr" style="display:none">
		<td width="10%" align=right>oracle 端口</td>
		<td><input type="text" size="30" name="sqlserver_port1" value="{{$p.sqlserver_port}}" /></td>
	  </tr>
	  {{assign var="trnumber" value=$trnumber+1}}
					<tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
...
{{if $pp.autologinflag eq 36}}
	document.getElementById('mysqlport').style.display='';
	document.getElementById('url').style.display='none';
{{elseif $pp.autologinflag eq 37}}
	document.getElementById('sqlserverport').style.display='';
	document.getElementById('url').style.display='none';
{{/if}}
}
