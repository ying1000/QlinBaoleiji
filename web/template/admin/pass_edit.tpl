...
<form name="f2" method=post action="admin.php?controller=admin_pro&action=pass_save&id={{$id}}&ip={{$ip}}&serverid={{$serverid}}&gid={{$gid}}&from={{$smarty.get.from}}" enctype="multipart/form-data" onsubmit="javascript:saveAccount=false;">
	...
	<tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}} id="logincommittr" >
		<TD width="20%" align=right>录入操作信息: </TD>
	...
	<tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}} id="publickey_authtr" >
		<TD width="20%" align=right>ssh认证方式: </TD>
				  ...
				  <select  class="wbk"  name="publickey_auth" onchange="changessh(this.value)">
					<OPTION VALUE="0" {{if $publickey_auth == 0}} selected {{/if}}>用户名密码</option>
					<OPTION VALUE="1" {{if $publickey_auth == 1}} selected {{/if}}>托管公私钥</option>
					<OPTION VALUE="2" {{if $publickey_auth == 2}} selected {{/if}}>透传公私钥</option>
				</select>&nbsp;&nbsp;&nbsp;公私钥
				<select  class="wbk" id="publickey" name="publickey" {{if $publickey_auth ne 1}}disabled{{/if}}>
				<OPTION VALUE="0">无</option>
				{{section name=p loop=$keys}}
					<OPTION VALUE="{{$keys[p].id}}" {{if $keys[p].id == $pubkey}} selected {{/if}}>{{$keys[p].sshkeyname}}</option>
				{{/section}}
				</select>
                </TR>    
     {{assign var="trnumber" value=$trnumber+1}}
 ...
 {{assign var="trnumber" value=$trnumber+1}}
	...
	<tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}} id="forwardtr" >
		<TD width="20%" align=right>进向加速: </TD>
                  <TD width="80%"><INPUT id="fastpath_input" {{if $fastpath_input == 1 or !$id}} checked {{/if}} type=checkbox name=fastpath_input value="on">  
                </TR>    
				 {{assign var="trnumber" value=$trnumber+1}}
	<tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}} id="outforwardtr" >
		<TD width="20%" align=right>出向加速: </TD>
                  <TD width="80%"><INPUT id="fastpath_output" {{if $fastpath_output == 1 or !$id}} checked {{/if}} type=checkbox name=fastpath_output value="on">  
                </TR>    
	...
	<tr {{if $trnumber % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
	  ...
	  <tr><td align="right" style="border-bottom:0px;border-top:0px;border-left:0px;border-right:0px;">全选<input type="checkbox" value=2 onclick="checkAll(this.checked);"></td></tr>
		...
		<td width="180"><input type="checkbox" id="uid_{{$allmem[g].uid}}" name='Check{{$smarty.section.g.index}}' value='{{$allmem[g].uid}}'  {{$allmem[g].check}}><a onclick="setSave();window.open ('admin.php?controller=admin_pro&action=passedit_seluser&uid={{$allmem[g].uid}}&sid={{$id}}&sessionluser={{$sessionluser}}', 'newwindow', 'height=550, width=400, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=yes,location=no, status=no');return false;" href="#" target="_blank" >{{if $allmem[g].binded}}<font color="red">{{/if}}{{$allmem[g].username}}({{if $allmem[g].realname}}{{$allmem[g].realname}}{{else}}未设置{{/if}}){{if $allmem[g].binded}}</font>{{/if}}</a></td>{{if ($smarty.section.g.index +1) % 5 == 0}}</tr><tr>{{/if}}
		{{/if}}
		{{/section}}
		</tr></table>
	  </td>
...
usernameselect();
changeport(0);
changessh({{if $publickey_auth}}{{$publickey_auth}}{{else}}0{{/if}});
