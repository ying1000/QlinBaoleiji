...
<TABLE width="670" align="center" border="0" cellspacing="0" cellpadding="0">
  ...
  <TBODY>
  <TR>
    <TD align="center" valign="top"><img src="{{$template_root}}/images/mobile_top{{if !($userinfo.usbkey and $userinfo.usbkeystatus eq 11)}}1{{/if}}.jpg" width="670" height="43"></TD>
  </TR>
  <TR>
...
<INPUT class="phonebtn3" id="ElapsedTimeBu" onClick="sendsms(120);" type="button" value="获取短信密码"></TD></TR>
 {{/if}}
 {{if $userinfo.emailauth}}
                    <TR>
                      <TD height="45" align="right">邮箱：</TD>
                      <TD id="red_text">{{$userinfo.email}}</TD></TR>
                    <TR>
...
function sendemail(seconds){
  $msg = '获取邮件密码';
  window.open ('admin.php?controller=admin_index&action=get_email&uid={{$userinfo.uid}}', 'hide', 'height=130, width=400, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=yes,location=no, status=no');
  document.getElementById('ElapsedTimeBu').disabled = true;
  document.getElementById('ElapsedTimeBu').value='还剩'+seconds+'秒';
  ElapsedTime(seconds)
