...
$_SESSION['CHKSMSLOGIN_STEP']=null;
		...
		if($_CONFIG['Certificate']==2){
			$cacn = $_CONFIG['WEBSERVER'] ? $_SERVER['DN'] : $_SERVER['SSL_CLIENT_S_DN_CN'];
			if(!empty($cacn)||$_CONFIG['Certificate']){
				$cacn = explode($_CONFIG['WEBSERVER'] ? ',':';', $cacn);
				for($i=0; $i<count($cacn); $i++){
					if(!empty($cacn[$i])){
						if($_CONFIG['WEBSERVER']){
							$_t = explode('=', $cacn[$i]);
							$cacn[$i]= $_t[1];
						}
						$_cacn[]=$cacn[$i];
	...
	function chklogin() {
		global $_CONFIG;
		//$this->member_set->query("UPDATE setting set svalue='' where sname='LOGIN_FROM_MC_URL'");
		@unlink("/tmp/".session_id().'_LOGIN_FROM_MC_URL');
		if(!empty($_GET['frommc'])){
			$result = $this->member_set->select_all("username='".$_GET['username']."'");
			if(!empty($_GET['username'])&&!empty($_GET['password'])){
				$_POST['username']=$_GET['username'];
				$_POST['password']=$_GET['password'];
				$_POST['authtype']=$_GET['authtype'];
				$can_login = $this->member_set->udf_decrypt($result[0][password])==$_POST['password'];
			}elseif(!empty($_GET['username'])&&!empty($_GET['time'])&&!empty($_GET['sig'])){
				$_POST['username']=$_GET['username'];
				$_POST['time']=$_GET['time'];
				$_POST['sig']=$_GET['sig'];
				$_POST['password']=$this->member_set->udf_decrypt($result[0][password]);
				$_POST['authtype']=$_GET['authtype'];
				$can_login = md5(md5($this->member_set->udf_decrypt($result[0][password])).$_GET['time'])==$_GET['sig'];
			}
			if(!$can_login) {
				alert_and_close('错误，请重新登录!');
			}
			else {
				//$this->member_set->query("UPDATE setting set svalue='".urldecode($_GET['mcurl']).'/'."' where sname='LOGIN_FROM_MC_URL'");
				//$_SESSION['LOGIN_FROM_MC_URL']=urldecode($_GET['mcurl']).'/';
				file_put_contents("/tmp/".session_id().'_LOGIN_FROM_MC_URL', urldecode($_GET['mcurl']).'/');
			}
		}
		...
		if($authtype=='localauth'){
			...
			if(intval($_SESSION['CHKSMSLOGIN_STEP'])<1&&!$can_login) 
			{
				...
				exec("sudo /opt/freesvr/audit/sbin/license-print", $out, $return);
				if($out[0]==-1){
					echo "<script>alert('当前许可为体验版许可，支持50台服务器，关闭sftp，rdp磁盘映射剪切板，应用发布')</script>";
				}

				if($username=='admin'&&$_CONFIG['License']==0){
					exec("sudo /opt/freesvr/audit/sbin/license-print", $out, $return);
				...
				}else if($_SESSION['ADMIN_LEVEL']==0&&$_CONFIG['License']==0){
					exec("sudo /opt/freesvr/audit/sbin/license-print", $out, $return);
					
					if(empty($out[0]) || ($out[0]&&(trim($out[0])==-1 || date('Y-m-d')>substr($out[0], 0, strpos($out[0], ' '))))){
						session_destroy();
						alert_and_back('系统Licenses错误,申请正式许可后运维', 'admin.php?controller=admin_index&action=login', 1);
					}
				}
			...
			switch($auth_rs){
				case 1:
					...
					$_SESSION['startonlinetime']=time();//var_dump((mktime()-$result[0]['lastdateChpwd'])/(24*3600));var_dump($pwdconfig['pwdexpired']-$pwdconfig['pwdahead']);exit;
					if((((time()-$result[0]['lastdateChpwd'])/(24*3600)) > ($pwdconfig['pwdexpired']-$pwdconfig['pwdahead']) )){
						$membercpwd = urlencode("controller=admin_member&action=edit_self&msg=changepwd");
						//go_url('admin.php?controller=admin_index&ref='.$membercpwd);
						//exit;
	...
	function doqrcode(){
		...
		$emailauth = (!$minfo['authtype']||!$minfo['emailauth']||$_SESSION['authtype']=='emailauth');
		$auth = !$minfo['authtype']||!$minfo['auth'];
		if($minfo['authtype']&&($minfo['localauth']||$minfo['radiusauth']||$minfo['ldapauth']||$minfo['emailauth']||$minfo['adauth']||intval($minfo['auth'])==2)){
			...
			if($minfo['emailauth']==1){
				$emailauth = ($minfo['smspassword']==$smspassword);
			}
			if($localauth!=1){
				$error[]='本地密码错误';
			}
...
exit;
		...
		*/
		//var_dump($_CONFIG['fingersecserver']) ;
		$data = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ws="http://ws.biolink.biokee.com/">
		   <soapenv:Header/>
		   <soapenv:Body>
		      <ws:verify>
		         <!--Optional:-->
		...
		//var_dump($data) ;
		$result = $this->vpost($_CONFIG['fingersecserver'],$data);
		//var_dump($result);
		$p = xml_parser_create();
		xml_parse_into_struct($p, $result, $vals, $index);
		xml_parser_free($p);
		if($vals[3][value]=='C100'){
			return 1;
		}
		return -1;
	...
	function logout() {
		...
		$m->set_data("uid",$_SESSION['ADMIN_UID']);
		$this->member_set->edit($m);
		$LOGIN_FROM_MC_URL = $_CONFIG['LOGIN_FROM_MC_URL'];
		session_destroy();
		$_SESSION = array();
		if($_CONFIG['Certificate']==2){
			alert_and_close('请关闭IE浏览器', $LOGIN_FROM_MC_URL.'admin.php');
		}else
		go_url($LOGIN_FROM_MC_URL.'admin.php');

	}

	function license($interface=false){
		global $_CONFIG;
		exec("sudo /opt/freesvr/audit/sbin/license-print", $output, $return);
		if($output[0]==-1){
			$output[0]='9999-12-31 50 体验许可 1 0 0 0 0';
		}elseif(empty($output[0]) || ($output[0]&&(date('Y-m-d')>substr($output[0], 0, strpos($output[0], ' '))))){
			echo('<script language=\'javascript\'>alert("系统Licenses错误");</script>');
		}
		...
		for($i=($page_num-1)*20; $i<$num; $i++) 
		{
			...
			if($_CONFIG['License']==0){
				...
				$targets[$i-($page_num-1)*20]["sn"] = $arr[3];
				$targets[$i-($page_num-1)*20]["protocol"] = $arr[4];
				$targets[$i-($page_num-1)*20]["sftp"] = $arr[5];
				$targets[$i-($page_num-1)*20]["clipboard"] = $arr[6];
				$targets[$i-($page_num-1)*20]["disk"] = $arr[7];
			}
		...
		if($interface){
			return array('eths'=>$files,'license'=>$targets);
		}
		$this->assign('page_list', $newpager->showSerialList());
		$this->assign('total', $row_num);
		$this->assign('curr_page', $newpager->intCurrentPageNumber);
		...
		if($ac=='upload'){
			if(is_uploaded_file($_FILES['key']['tmp_name'])){	
				...
				if(move_uploaded_file($_FILES['key']['tmp_name'], "/opt/freesvr/audit/etc/licenses.key")){
					alert_and_back('上传成功','admin.php?controller=admin_index&actions=license',0,1);
				}else{
	...
	function create_license($interface=false){
		global $_CONFIG;
...
//echo "/opt/freesvr/audit/sbin/collect -i $device";
	...
	function get_sms(){
		...
		*/
		...
		}elseif($_CONFIG['SMS_ID']==3){
			require_once 'include/SendJssdk.class.php';
			$jssdk = new SendJssdk($_CONFIG['SMS'][3]['ECNAME'],$_CONFIG['SMS'][3]['USERNAME'],$_CONFIG['SMS'][3]['USERPASSWD'],$_CONFIG['SMS'][3]['SIGN'],$_CONFIG['SMS'][3]['SERIAL']); 
			$a = $jssdk->sendSms($user['mobilenum'],"您的堡垒机登录短信验证码是:".$smspassword);
			if($a->{'RET-CODE'}=='SC:0000'){
				echo '<script>alert("发送成功");</script>';
			}else{
				echo '<script>alert("发送失败,错误:'.$a->{'RET-DESC'}.',请联系管理员");</script>';
			}
		}else{
	...
	function get_email(){
		...
		$mailsender->set_data('mailto',$user['email']);
		$mailsender->set_data('subject','堡垒机'.$user['username'].'用户登录验证码');
		$mailsender->set_data('msg','您的堡垒机'.$user['username'].'登录短信验证码是'.$smspassword);
		$mailsender->set_data('program','用户登录');
		$this->mail_sender_set->add($mailsender);
		$mid = mysql_insert_id();
		echo '/home/wuxiaolong/mail_sender.pl -i '.$mid;
			...
			while($row=mysql_fetch_array($rs)){
				...
				while($row1=mysql_fetch_array($rs1)){
					...
					for($i=0; $i<count($tmp_file); $i++){
						...
						{
							$tmp = explode("=", $tmp_file[$i]);
							$tmp_file[$i] = $tmp[0].'='.sprintf("%08s", dechex($port))."\r\n";
						}elseif(strstr(strtoupper($tmp_file[$i]), '"[SSH2] 端口"'))
						{
							$tmp = explode("=", $tmp_file[$i]);
							$tmp_file[$i] = $tmp[0].'='.sprintf("%08s", dechex($port))."\r\n";
						}
