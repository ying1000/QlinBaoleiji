...
class c_admin_pro extends c_base {
	function dev_index($interface=false) {
		...
		$sql = "SELECT servers.*,".($interface ? "NULL superpassword," : "")."(UNIX_TIMESTAMP(servers.asset_warrantdate)-UNIX_TIMESTAMP())/(60*60*24) warrantdays,dd.userct,
	...
	function devbatchdel() {
		...
		$this->assign("orderby2", $orderby2);
		$sql = "SELECT servers.*,(UNIX_TIMESTAMP(servers.asset_warrantdate)-UNIX_TIMESTAMP())/(60*60*24) warrantdays,dd.userct,
					(SELECT COUNT(is_modified) FROM ".$this->backup_set->get_table_name()." b 
						WHERE b.device_id IN (
							SELECT id FROM ".$this->devpass_set->get_table_name()." d 
								WHERE d.device_ip=servers.device_ip 
							) AND b.is_modified=1
					) ct FROM ".$this->server_set->get_table_name()." servers LEFT JOIN (SELECT COUNT(*) AS userct, device_ip FROM ".$this->devpass_set->get_table_name()." GROUP BY device_ip) dd ON servers.device_ip=dd.device_ip  
						WHERE $where ORDER BY ".$orderby1." $orderby2";
	...
	function dev_del($interface=false) {
		$id = get_request('id');
		$gid = get_request('gid');
		$id_arr = array();
		if($id){
...
*/
	...
	function insertrandom($ip, $device_ip, $username, $luser, $time, $code){
		...
		$sql = "INSERT INTO random SET device_ip='".$device_ip."', username='".$username."', luser='".addslashes($luser)."', time='".$time."', code='".$code."', inputusername='".addslashes($_SESSION['LOGIN_USERNAME'])."', inputpassword=".(!$_CONFIG['PASSWORD_ENCRYPT_TYPE'] ? "udf_encrypt('".($_SESSION['LOGIN_PASSWORD'])."')" : "AES_ENCRYPT('".($_SESSION['LOGIN_PASSWORD'])."','".$_CONFIG['PASSWORD_KEY']."')").", logincommit='".intval($_SESSION['COMMITID'])."'";
	...
	function dev_login() {
		...
		$outsideip = get_request('outsideip', 0, 1);
		$showdomain = get_request('showdomain', 0, 1);
		$folder = get_request('folder', 0, 1);
		...
		setcookie("outsideip", $outsideip, time()+3600*24*365*100, '/', '', 1);
		...
		if($dev['entrust_username']==0&&strtolower($tem['login_method']) != 'apppub'){
			if(!in_array($id,$_SESSION['INPUTDEVICESIDS'])){
				...
				$_continue = ($blankuser || (strtolower($tem['login_method']) == 'ftp'||(strtolower($tem['login_method']) == 'ssh'&&($logintool=='winscp'||$logintool=='flashxp'||$logintool=='xftp'||$logintool=='filezilla'))));
		...
		if($showdomain){
			$this->assign("hostname", iconv("UTF-8", "GB2312", $dev['hostname']));
		}
		if($type=='gateway' || $type=='fort' ){
			$this->assign("dynamic_pwd",'');
		}
		$tem[login_method] = ($tem[login_method]=='ssh' && $dev['sftp']==1&&($logintool=='winscp'||$logintool=='filezilla') ? 'sftp' : $tem[login_method]);
		$this->assign('login_method', $tem[login_method]);
		$eth0tmp = explode(":", $_SERVER["HTTP_HOST"]);
		if($_CONFIG['APP_HOST']=='127.0.0.1'){

			$_CONFIG['APP_HOST']=$eth0tmp[0];
		}
		if(empty($selectedip)){
			...
			*/
			...
			if($rdptype=='activex'){
				...
				if(strtolower($tem['login_method']) == 'web'){
					...
					$_appserver = $this->appserver_set->select_all("appserverip='".$dev['device_ip']."'");
					$str = genRandomString(8);
					...
					$this->assign('username',$user['username'].'--'.$dev[id].'--'.$str);
					$this->assign('password',$str.'--');
					$this->assign('host', ($outsideip ? $_appserver[0]['outsideip'] : $dev['device_ip']));
					...
					$this->assign('param', $_SESSION['ADMIN_UID'].($appdeviceid>0 ? '-'.$appdeviceid : '').' '.$dev['device_ip'].(empty($_CONFIG['RDPPLAYIP']) ? ' '.$_CONFIG['APP_HOST'] : ' '.$_CONFIG['RDPPLAYIP']).' '.$_SERVER['REMOTE_ADDR']);
					$this->assign("rdpclientversion", $_CONFIG['RdpClientVersion']);
					if($folder){
						$this->assign('path', $folder);
						$this->assign('param', '');
					}
				}
			...
			if(isset($_GET['crttab'])){
				$this->assign('crttab',$_GET['crttab']);	
			}
			$this->display('devlogin.tpl');
		}
	}
	...
	function pass_edit() {
		...
		if($id != 0) {
			...
			$sshkey = $this->sshkey_set->select_all("devicesid=".$id);
			...
			$this->assign('pubkey', $sshkey[0]['sshkeyname']);
...
//echo '<pre>';print_r($_SESSION);echo '</pre>';
		...
		$this->assign('sessionluser', $sessionluser);
		$this->assign('sessionlgroup', $sessionlgroup);
		$this->assign("keys", $this->sshkeyname_set->select_all());
		$this->assign('fromdevpriority', $fromdevpriority);
	...
	function pass_save($interface=false) {
		...
		$publickey_auth = get_request('publickey_auth',1,0);
		$pubkey = get_request('publickey',1,0);
		...
		if($id == 0) {
			if(0 == $this->devpass_set->select_count("username = '$new_name' and device_ip='$ip' and port='$port' and hostname='".$server['hostname']."' and login_method='$login_method'")) {
				...
				if($publickey_auth==2){
					$new_pass->set_data("agent", 1);
				}else{
					$new_pass->set_data("agent", 0);
				}
				if($interface){
					$new_pass->set_data('interface',1);
				}
				if($enable == 'on') {
				...
				if($id&&$pubkey&&$this->sshkey_set->select_count("devicesid=$id and sshkeyname=$pubkey")<=0){
					$sshkey = new sshkey();
					$sshkey->set_data('sshkeyname',$pubkey);
					$sshkey->set_data('devicesid',$id);
					$sshkey->set_data('memberid',1);
	...
	private function luser_group_save($sessionluser, $sessionlgroup, $sid, $newuser, $alluser, $newgroup,$allgroup, $no=0){
		...
		for($i=0; $i<count($_SESSION[$sessionluser]); $i++){
			if(in_array($_SESSION[$sessionluser][$i]['memberid'], $newuser)){
				...
				if(isset($_SESSION[$sessionluser][$i]['forbidden_commands_groups']))
				$luser->set_data('forbidden_commands_groups', $_SESSION[$sessionluser][$i]['forbidden_commands_groups']);
				$luser->set_data('smsalert', $_SESSION[$sessionluser][$i]['smsalert']);
				$luser->set_data('workflow', $_SESSION[$sessionluser][$i]['workflow']);
				$luser->set_data('wf_user1', $_SESSION[$sessionluser][$i]['wf_user1']);
		...
		for($i=0; $i<count($_SESSION[$sessionlgroup]); $i++){
			if(in_array($_SESSION[$sessionlgroup][$i]['groupid'], $newgroup)){
				...
				if(isset($_SESSION[$sessionlgroup][$i]['forbidden_commands_groups']))
				$lgroup->set_data('forbidden_commands_groups', $_SESSION[$sessionlgroup][$i]['forbidden_commands_groups']);		
				...
				//var_dump($_SESSION[$sessionlgroup]);exit;
				if($_SESSION[$sessionlgroup][$i]['id']){
					$lgroup->set_data('id', $_SESSION[$sessionlgroup][$i]['id']);
	...
	function passwordedit(){
		...
		if($ac == 'doit'){
			...
			$ha = $ha[0];
			if(strlen($password)<$ha['minlen']){
				alert_and_back('密码最小长度为:'.$ha['minlen']);			
				return ;
			}
			$alphas = preg_match_all('/[a-zA-Z]/', $password, $matches);
			if(intval($ha['minalpha'])>$alphas){
				alert_and_back('包含字母数最少为:'.$ha['minalpha']);
				return ;
			}
			$others = strlen($password)-$alphas;
			if($others<$ha['minother']){
				alert_and_back('包含其他数最少为:'.$ha['minother']);			
				return ;
			}
			$max=0;
			for($i=0; $i<strlen($password); $i++){
				if(!isset($char[$password[$i]])) $char[$password[$i]]=0;
				$char[$password[$i]]++;
				if($char[$password[$i]]>$max) $max=$char[$password[$i]];
			}
			if($max>$ha['maxrepeats']){
				alert_and_back('最大重复字符数:'.$ha['maxrepeats']);			
				return ;
			}
...
<?php  
   ...
   sleep(2);
		...
		$this->assign("pwdconfig_pwdstrong1", intval($pwdconfig['minalpha']));
		$this->assign("pwdconfig_pwdstrong2", intval($pwdconfig['minother']));
		//$this->assign("pwdconfig_pwdstrong3", intval($pwdconfig['pwdstrong3']));
		//$this->assign("pwdconfig_pwdstrong4", intval($pwdconfig['pwdstrong4']));
		$this->assign("pwdconfig_login_pwd_length", intval($ha['minlen']));
		$this->assign("_config", $_CONFIG);
	...
	function dev_priority_search(){
		...
		$where = "1";
		if($ip){
			$where .= " AND (devices.device_ip like '%$ip%' or devices.hostname like '%$ip%')";
		}
		if($s_user){
			$where .= " AND devices.username like '%$s_user%'";
		}
		if($user){
			$where .= " AND (member.username like '%$user%' or member.realname like '%$user%')";
		}
	...
	function app_priority_search(){
		...
		if($device_ip != '') {
			$where .= " AND (appdevices.device_ip LIKE '%$device_ip%' or ss.hostname  LIKE '%$device_ip%')";
		}
	...
	function resource_group($interface=false) {
		...
		if($username){
			$where .= ' AND id IN(select resourceid FROM '.$this->luser_resourcegrp_set->get_table_name().' WHERE memberid IN (select uid from '.$this->member_set->get_table_name().' where username like "%'.($username).'%" or realname like "%'.($username).'%") UNION select resourceid FROM '.$this->lgroup_resourcegrp_set->get_table_name().' WHERE groupid IN (select groupid from '.$this->member_set->get_table_name().' where username like "%'.($username).'%" or realname like "%'.($username).'%"))';
		}elseif($groupid){
	...
	function resource_group_edit() {
		...
		if($_POST){
			$selectgroup  = array(0);
			$selectdevice = array(0);
			for($i=0; $i<count($_POST['selected']); $i++){
				$selected_arr = explode('_', $_POST['selected'][$i]);
				if($selected_arr[0]=='groupid'){
					$selectgroup[]=$selected_arr[1];
				}else{
					$selectdevice[]=$_POST['selected'][$i];
				}
			}
			$res = $this->devpass_set->base_select("SELECT d.*,l.login_method lmname,if(entrust_password=1,'托密','不托密') as ep FROM devices d LEFT JOIN login_template l ON d.login_method=l.id LEFT JOIN (SELECT * FROM servers WHERE 1 ".($_SESSION['ADMIN_LEVEL']==3|| $_SESSION['ADMIN_LEVEL']==21 || $_SESSION['ADMIN_LEVEL']==101 ? ($_SESSION['ADMIN_MSERVERGROUP']==0 ? ' AND 0' : '').' AND (groupid IN('.implode(',', $_SESSION['ADMIN_MSERVERGROUP_IDS']).'))' : '').") s ON d.device_ip=s.device_ip  WHERE d.id IN(".implode(',', $selectdevice).") AND s.id IS NOT NULL ");

			$res2 = $this->devpass_set->base_select("SELECT a.groupid,a.username,b.groupname FROM ".$this->resgroup_set->get_table_name()." a left join ".$this->sgroup_set->get_table_name()." b ON a.groupid=b.id WHERE b.id IN(".implode(',', $selectgroup).") AND a.groupname='".$ginfo['groupname']."' and devicesid=-1 ".($_SESSION['ADMIN_LEVEL']==3|| $_SESSION['ADMIN_LEVEL']==21 || $_SESSION['ADMIN_LEVEL']==101 ? ($_SESSION['ADMIN_MSERVERGROUP']==0 ? ' AND 0' : '').' AND (groupid IN('.implode(',', $_SESSION['ADMIN_MSERVERGROUP_IDS']).'))' : '')." ");
		}else{
	...
	function resource_group_save($interface=false) {
		...
		if($gid){
			$oldgroup = $this->resgroup_set->select_by_id($gid);
			$oldgname = $oldgroup['groupname'];
			$_ct=$this->resgroup_set->select_count("groupname='$oldgname' and devicesid!=0");
		...
		$sgroup_arr = array();
		$insertsql = "INSERT INTO resourcegroup(groupname,devicesid,username,user,groupid) values ";
		...
		if($sgroup_arr){
			$this->resgroup_set->query("INSERT INTO resourcegroup(groupname,devicesid,username,user,groupid) select '".$gname."',-1,'0','".($_SESSION['ADMIN_LEVEL']==1 ? $createuser : $_SESSION['ADMIN_USERNAME'])."',id from servergroup sg1 where locate(concat(',',id,','), (select concat(',',group_concat(ifnull(child,0)),',') from servergroup where id IN(".implode(',',$sgroup_arr)."))) and not exists(select * from resourcegroup where groupname='".$gname."' and groupid=sg1.id and devicesid=-1)");
			$sql = "select d.id from devices d left join servers s on d.device_ip=s.device_ip left join servergroup sg on s.groupid=sg.id where sg.id IN(".implode(',',$sgroup_arr).")";
		...
		$this->resourcegroup_bindsave($gid, $changed);
...
//echo '<pre>';print_r($_SESSION);echo '</pre>';
	...
	function sshkey_delete(){
		...
		if(empty($id)){
			$id = $_POST['chk_member'];
			if($id&&$this->sshkey_set->select_count("sshkeyname in('".implode('\',\'', $id)."')")>0){
	...
	function test_port($interface=false){
		...
		if($devicesid){
			...
			if($interface){
				$dev['result']=($return==1 ? '连接成功' : '连接不成功');
				return $dev;
			}
			$this->assign("c", $dev);
			...
			if($return==1){
				if($interface){
					return $dev['result']='连接成功';
				}
				alert_and_back('连接成功');
				exit;
			}else{
				if($interface){
					return $dev['result']='连接不成功';
				}
